...
const Indexes = () => {
            ...
            setData((prevData) => ({
                ...
                services: prevData.services.map((service) => ({
                    ...
                    cpu_pressure_index: Math.min(
                        1,
                        Math.max(0, service.cpu_pressure_index + (Math.random() * 0.1 - 0.05))
                    ),
                    ...
                    io_pressure_index: Math.min(
                        1,
                        Math.max(0, service.io_pressure_index + (Math.random() * 0.1 - 0.05))
                    ),
    ...
    return (
        <div className="flex flex-col flex-1 gap-6 p-6">
                ...
                <div className="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-blue-700 dark:text-blue-300">
                            Pressure indexes combine multiple resource metrics (CPU, memory, I/O)
                            with graph centrality data to predict service stress. The stress
                            index aggregates all pressure signals to recommend optimal replica
                            counts, preventing both over-provisioning and resource starvation.
                        </p>
                    </div>
                </div>
            </div>
            ...
            {/* Graph */}
            <IndexesGraph data={data.historical} />
