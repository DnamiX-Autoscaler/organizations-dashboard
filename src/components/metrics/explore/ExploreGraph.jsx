...
// Move helper functions above useMemo
...
const ExploreGraph = ({ queryData, queryExpression }) => {
  ...
  const metricKeys = useMemo(() => {
    ...
    return Object.keys(graphData[0]).filter(
      (key) => key !== "time" && key !== "timeStamp"
    );
  }, [graphData]);

  // Colors for multiple series - vibrant colors for better visualization
  const seriesColors = [
    "#a100ff", // Purple/Magenta
    "#ff006e", // Hot Pink
    "#00d9ff", // Cyan
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-3 p-4 border border-blue-200 rounded-lg bg-blue-50 dark:bg-blue-900/20 dark:border-blue-800">
        ...
        <div className="flex-1">
          ...
          <p className="mt-1 font-mono text-xs text-blue-600 break-all dark:text-blue-300">
            {queryExpression}
          </p>
          ...
          <div className="p-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-darkBackgroundVery">
            ...
            <div className="space-y-3">
              {queryData?.data?.result?.map((series, idx) => (
                <div
                  key={idx}
                  className="p-3 bg-white border border-gray-200 rounded-lg dark:bg-darkBackground dark:border-gray-700"
                >
          ...
          {metricKeys.length > 0 ? (
            metricKeys.map((metricKey, index) => (
              <div key={metricKey} className="space-y-2">
                <div className="flex items-center gap-2 px-4 py-2 rounded-lg bg-gray-50 dark:bg-darkBackgroundVery">
                  <div
                ...
                <Graph
                  ...
                  xKey="time"
                  yKey={metricKey}
                  chartType="area" // Always show area/gradient by default
