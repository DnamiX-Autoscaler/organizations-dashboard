...
const RateBar = ({ value, max = 100, color }) => {
    ...
    const barColor =
        color === "success"
            ? pct < 80 ? "bg-red-500" : pct < 95 ? "bg-yellow-400" : "bg-emerald-500"
            : pct >= 5 ? "bg-red-500" : pct >= 1 ? "bg-yellow-400" : "bg-blue-400";
    return (
    ...
    return (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {data.map((item, idx) => {
                ...
                const hasErrors = errorRate > 0 || parseFloat(item.http_5xx_rate_percent) > 0;
                ...
                return (
                    ...
                    >
                        {/* Card Header */}
                        <div className="flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-indigo-50 to-blue-50 dark:from-indigo-900/20 dark:to-blue-900/20 border-b border-gray-200 dark:border-gray-700">
                            <div className="flex items-center justify-center w-9 h-9 bg-indigo-100 dark:bg-indigo-900/40 rounded-lg shrink-0">
                                <Icon icon="mdi:application" className="w-5 h-5 text-indigo-600 dark:text-indigo-400" />
                            </div>
                            ...
                            {hasErrors && (
                                <span title="Errors detected">
                                    <Icon icon="mdi:alert-circle" className="w-5 h-5 text-red-500 shrink-0" />
                                </span>
                            )}
                        </div>
                        ...
                        <div className="flex flex-col gap-3 p-4">
                            ...
                            <div className="flex items-center justify-between">
                                <span className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400">
                                    <Icon icon="mdi:transfer" className="w-3.5 h-3.5 text-blue-500" />
                                    Request Rate
                                </span>
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-0.5">
                                    <span className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400">
                                        <Icon icon="mdi:check-circle-outline" className="w-3.5 h-3.5 text-emerald-500" />
                                        Success Rate
                                    </span>
                                    <span className={`text-xs font-bold ${successRate >= 95 ? "text-emerald-600 dark:text-emerald-400" : successRate >= 80 ? "text-yellow-500" : "text-red-500"}`}>
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-0.5">
                                    <span className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400">
                                        <Icon icon="mdi:alert-circle-outline" className="w-3.5 h-3.5 text-red-400" />
                                        Error Rate
                                    </span>
                                    <span className={`text-xs font-bold ${errorRate > 1 ? "text-red-500" : "text-gray-500 dark:text-gray-400"}`}>
                                        {errorRate.toFixed(2)}%
                                        <span className="ml-2 font-normal text-gray-400">
                                            4xx {parseFloat(item.http_4xx_rate_percent || 0).toFixed(2)}%
                                            &nbsp;5xx {parseFloat(item.http_5xx_rate_percent || 0).toFixed(2)}%
                                        </span>
                                    </span>
                                </div>
                            ...
                            <div>
                                <p className="flex items-center gap-1.5 text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">
                                    <Icon icon="mdi:timer-outline" className="w-3.5 h-3.5 text-amber-500" />
                                    Latency
                                </p>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                ...
                                <StatBox
                                    icon="mdi:gauge"
                                    iconColor={saturation >= 80 ? "text-red-500" : "text-teal-400"}
                                    label="Saturation"
